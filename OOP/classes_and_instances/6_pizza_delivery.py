class PizzaDelivery:
    ...
    def pizza_ordered(self):
        if not self.ordered:
            self.ordered = True
            self.ingredients = dict(sorted(self.ingredients.items(), key=lambda x: (-x[1], x[0])))
            temp = ", ".join({f"{k}: {v}" for k, v in self.ingredients.items()})
            return f"You've ordered pizza {self.name} prepared with {temp} and the price will be {self.price}lv."
